...
void main()
{
    ...
    vec4 leafcol = texture2D(leaf, 3.*gl_TexCoord[0].xy);
    vec4 color = texture2D(sky, gl_TexCoord[1].xy) + leafcol * leafcol.a;
    ...
    if (perFragCalc == 1)
    {
        ...
        vec4 leafcol = texture2D(leaf, 3.f*gl_TexCoord[0].xy);
